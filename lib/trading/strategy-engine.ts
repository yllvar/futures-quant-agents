// Moving from lib/strategy-engine.ts
import type { MarketData, StrategyConfig, MarketRegime, Timeframe } from "../core/types"
    ...
    return [
      {
        ...
        signalValidation: {
          ...
          confirmationIndicators: ["macd", "ema20", "sma50", "adx"],
          minimumConfirmations: 3, // At least 3 indicators must confirm
          consecutiveSignals: 2, // Require 2 consecutive matching signals
          volumeThreshold: 10, // 10% volume increase
          regimeConsistency: true, // Signal must match market regime
        },
      },
      {
        id: "mean-reversion",
        ...
        signalValidation: {
          confidenceThreshold: 0.75, // Require 75% confidence
          confirmationIndicators: ["rsi", "bollinger", "stochastic"],
          minimumConfirmations: 2, // At least 2 indicators must confirm
          consecutiveSignals: 1, // Only require 1 signal (mean reversion can be quick)
          volumeThreshold: undefined, // Volume not as important for mean reversion
          regimeConsistency: true, // Signal must match market regime
        },
      },
      {
        ...
        signalValidation: {
          confidenceThreshold: 0.8, // Require 80% confidence (breakouts need higher confidence)
          confirmationIndicators: ["donchian", "volume", "adx"],
          minimumConfirmations: 2, // At least 2 indicators must confirm
          consecutiveSignals: 1, // Only require 1 signal (breakouts are often singular events)
          volumeThreshold: 30, // 30% volume increase (breakouts need volume confirmation)
          regimeConsistency: true, // Signal must match market regime
        },
      },
      {
        ...
        signalValidation: {
          confidenceThreshold: 0.65, // Lower confidence threshold for volatile markets
          confirmationIndicators: ["ema20", "atr", "macd"],
          minimumConfirmations: 2, // At least 2 indicators must confirm
          consecutiveSignals: 2, // Require 2 consecutive matching signals
          volumeThreshold: 15, // 15% volume increase
          regimeConsistency: false, // Volatile markets can have mixed signals
        },
      },
    ]
  }
